...
def show_pattern(x, datasets):
    print('--eval process--')
    model = net.to(Device)
    model.load_state_dict(torch.load('./pts/GSNET_{}.pt'.format(datasets)))
    model.eval()
    data_eval = DataLoader(dataset=x, batch_size=200, shuffle=False)
    ...
    y1 = temp_data[1:, :]        
    y_np = y1.numpy()
    plot_embedding_2d(y_np, x.targets.data.numpy(), 'GSNET_{}'.format(args.dataset)) 
...
if __name__ == '__main__':
    ...
    # load data
    data_structure, x = load_data(args.dataset)
    
    if args.mode == 'train':
        gsnet(x, args.batch_size, args.epochs)
        show_pattern(data_structure, args.dataset)
